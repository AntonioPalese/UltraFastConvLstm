...
void visualize(string &full_im_name, vector<vector<Point2f> > &anno_lanes, vector<vector<Point2f> > &detect_lanes, vector<int> anno_match, int width_lane, string &save_path);


bool check_existence(string &sub_im_name, string &detect_dir){
...
int main(int argc, char **argv)
{
  ...
  while (getline(ifs_im_list, sub_im_name)) {
	if(check_existence(sub_im_name, detect_dir))
	{
...
#pragma omp parallel for
	...
	{
		auto sub_im_name = trim(filelists[i]);
		...
		if (show)
		{
			...
			auto save_path = detect_dir+sub_im_name;
			visualize(full_im_name, anno_lanes, detect_lanes, anno_match, width_lane, save_path);
			//waitKey(0);
		}
	}
...
void visualize(string &full_im_name, vector<vector<Point2f> > &anno_lanes, vector<vector<Point2f> > &detect_lanes, vector<int> anno_match, int width_lane, string &save_path)
{
	...
	string save_path_gt  = save_path.substr(0, save_path.find_last_of(".")) + ".gt" + ".jpg";
	imwrite(save_path_gt, img);
