...
def _get_index(label, row_anchors,num_lanes, himg):
    w,h=label.size

    if h != himg:
        scale_f=lambda x: int((x * 1.0 / himg) * h)
        sample_tmp=list(map(scale_f,row_anchors))

    all_idx=np.zeros((num_lanes,len(sample_tmp),2))
...
class LaneClsDataset(torch.utils.data.Dataset):
    def __init__(self,cfg,img_transform=None, 
        ...
        if mode == 'train':
            list_path = cfg.train.train_list_path
        elif mode == 'validation':
            list_path = cfg.validate.val_list_path
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len):            
            l = self.list[index*self.seq_len + i]
            l_info = l.split(' ')
            ...
            lane_pts = _get_index(label, self.row_anchors, self.num_lanes, self.size[0]) # row number of the resized image is self.size[0] 

            # get the coordinates of lanes at row anchors

            w, h = img.size
            cls_label = _grid_pts(lane_pts, self.griding_num, w) 
            # w is the number of the columns in the original image
