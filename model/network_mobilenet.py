...
class parsingNet(torch.nn.Module):
    def __init__(self,cfg):
        ...
        self.cls_dim = tuple(cfg.model.cls_dim) # (num_gridding + 1, num_cls_per_lane, num_of_lanes)
        ...
        '''if self.use_aux:
            ...
            self.initialize_weights(self.aux_header2,self.aux_header3,self.aux_header4,self.aux_combine)'''
        ...
        self.pool_before_lstm = torch.nn.Sequential(
            torch.nn.Conv2d(1280,256,1),
            torch.nn.ReLU(),
        )
        self.pool_after_lstm = torch.nn.Sequential(
    ...
    def forward(self, x):
        ...
        for x in unbinded:
            _,_,_,fea = self.model(x)
            fea = self.pool_before_lstm(fea)
            '''if self.use_aux:
                lasts_of_the_sequence = (x2, x3)'''
            outs.append(fea)


        '''if self.use_aux:
            x2, x3 = lasts_of_the_sequence
        ...
        #print('shape after backbone : ', stacked_sequence.shape) #torch.Size([32, 3, 512, 9, 25])
        ...
        #torch.Size([32, 512, 9, 25])

        o=self.pool_after_lstm(test).view(-1,8 * 9 * 25)
        #print('shape after pooling layer an flattening ', o.shape) #torch.Size([32, 1800])
        o=self.cls(o).view(-1,*self.cls_dim)
        #print('shape after classification layer and reviewing ', o.shape) #torch.Size([32, 101, 18, 4])
...
class ConvLSTMnode(torch.nn.Module):
    def __init__(self, input_size, channel_input, hidden_size, kernel_size, bias, device):
        ...
        self.conv = torch.nn.Conv2d(in_channels=self.channel_input + hidden_size,
                                    ...
                                    bias=self.bias,padding=self.padding, device=self.device)
